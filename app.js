...
// -----------------------------------------------------------------------------
// App overview
// This file contains shared storage helpers, UI templates, and page renderers.
// Page renderers are organized below by feature and mapped in initPage().
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Shared navigation / routing helpers
// -----------------------------------------------------------------------------
function renderNav() {
...
function getRequestDetailHref(id) {
  const currentPath = window.location.pathname || "";
  return `${currentPath.includes("/pages/") ? "../../request-detail.html" : "request-detail.html"}?id=${id}`;
}

// -----------------------------------------------------------------------------
// Shared UI templates / request card builders
// Used by landing, dashboard, explore, and other list views.
// -----------------------------------------------------------------------------
  ...
  return `
    <article class="request-card fade-in">
      ...
      <div class="list-item" style="padding-bottom:0;border-bottom:0;">
        ...
        <a class="btn btn-secondary" href="${getRequestDetailHref(request.id)}">Open details</a>
      </div>
    </article>
  `;
}
